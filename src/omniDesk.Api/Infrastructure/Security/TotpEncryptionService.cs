...
public sealed class TotpEncryptionService
{
    private readonly byte[] _key;

    public TotpEncryptionService()
    {
        var keyBase64 = Environment.GetEnvironmentVariable("TOTP_ENCRYPTION_KEY")
            ?? throw new InvalidOperationException("TOTP_ENCRYPTION_KEY is not configured.");

        _key = Convert.FromBase64String(keyBase64);

        if (_key.Length != 32)
            throw new InvalidOperationException("TOTP_ENCRYPTION_KEY must be 32 bytes (256-bit) when decoded from Base64.");
    }
